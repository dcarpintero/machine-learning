...
RLHF (Reward Learning from Human Feedback) is a fine-tuning process used to align Language Models (LLMs) with human preferences. It involves assessing LLM completions using a reward model based on human preference metrics like helpfulness. Through the Proximal Policy Optimization (PPO) algorithm, the weights of the LLM are updated according to the assigned rewards. This iterative process is repeated with different prompts and weight updates until the desired alignment is achieved, resulting in a human-aligned LLM.
...
To address reward hacking, KL divergence (Kullback-Leibler divergence) is used. KL divergence is a statistical measure of how different two probability distributions are. In the context of RLHF, KL divergence is used to compare the completions generated by the updated LLM with a reference LLM. By comparing the two completions, the degree of divergence of the updated model from the reference can be determined.

During training, each prompt is passed to both the reference LLM (with frozen weights) and the updated LLM. The completions generated by both models are compared using KL divergence. The KL divergence is calculated for each token across the LLM's vocabulary. By using a softmax function, the number of probabilities is reduced, making the calculation less computationally expensive. The KL divergence calculation provides insight into how much the updated LLM has diverged from the reference LLM. By penalizing significant divergences, RLHF discourages reward hacking and encourages the updated model to stay closer to the original language model's behavior.
